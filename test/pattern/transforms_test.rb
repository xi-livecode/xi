...
describe Xi::Pattern::Transforms do
  describe '#-@' do
    it 'returns a new pattern with inverted numbers' do
      @p = -(1..5).p
      assert_equal [-1, -2, -3, -4, -5], @p.to_v
    end

    it 'preserves non-numeric values' do
      @p = -[1, 42, 'w', [4, 5], [10].p].p
      assert_equal [-1, -42, 'w', [4, 5], -10], @p.to_v
    end
  end
  ...
  describe '#+' do
    ...
    describe 'when RHS has #+ defined' do
      it 'performs a scalar sum' do
        @p = (1..5).p + 100
        assert_equal [101, 102, 103, 104, 105], @p.to_v
      end
    end
  end
  ...
  describe '#%' do
    describe 'when RHS has #/ defined' do
      it 'performs a scalar floating-point division' do
        @p = (1..7).p % 2
        assert_equal [1, 0, 1, 0, 1, 0, 1], @p.to_v
      end
    end
  end
  ...
  describe '#seq' do
    ...
    it 'cycles sequentially :repeats times' do
      @p = [1, 2, 3].p.seq
      assert_equal [1, 2, 3], @p.to_v

      @p = [1, 2, 3].p.seq(2)
      assert_equal [1, 2, 3, 1, 2, 3], @p.to_v
    ...
    it 'cycles forever if :repeats == :inf' do
      @p = [1, 2, 3].p.seq(:inf)
      assert_equal [1, 2, 3, 1, 2, 3, 1, 2, 3, 1], @p.take_v(10)
    end
    ...
    it 'cycles the pattern with a different starting offset' do
      ...
      @p = (1..5).p.seq(1, 2)
      assert_equal [3, 4, 5, 1, 2], @p.to_v

      @p = (1..5).p.seq(1, 4)
      assert_equal [5, 1, 2, 3, 4], @p.to_v
  ...
  describe '#denormalize' do
    it 'scales back normalized values to a custom range' do
      @p = [0, 0.25, 0.50, 0.75].p.denormalize(0, 0x100).to_v
  ...
  describe '#scale' do
    it 'scales values from a range to another' do
      @p = [0, 1, 2, 3].p.scale(0, 4, 0, 0x80).to_v
